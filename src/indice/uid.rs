...
pub fn to_uid(term: &Term<BareID>, uid_generator: &mut UIDGenerator) -> Term<UID> {
    _to_uid(term, uid_generator, HashMap::default())
}

fn _to_uid(term: &Term<BareID>, uid_generator: &mut UIDGenerator, mut free_vars: HashMap<char, usize>) -> Term<UID> {
    match term {
        Var(chr) => Var(UID{name: *chr, uid: if free_vars.contains_key(chr) {free_vars[chr]} else {uid_generator.next()}}),
        Abs(bound, term) => {
